...
class MyFileDatabaseUnitTests : public ::testing::Test {
 protected:
  ...
  static void SetUpTestSuite() {
    testDatabase = new MyFileDatabase(1, "unittestsfile.bin");
    ...
    auto econ1105 = std::make_shared<Course>(210, "Waseem Noor",
                                            locations[1], times[3]);
    econ1105->setEnrolledStudentCount(187);
    auto econ2257 = std::make_shared<Course>(125, "Tamrat Gashaw",
                                            "428 PUP", times[2]);
    econ2257->setEnrolledStudentCount(63);
    auto econ3211 = std::make_shared<Course>(96, "Murat Yilmaz",
                                            "310 FAY", times[1]);
    econ3211->setEnrolledStudentCount(81);
    auto econ3213 = std::make_shared<Course>(86, "Miles Leahey",
                                            "702 HAM", times[1]);
    econ3213->setEnrolledStudentCount(77);
    auto econ3412 = std::make_shared<Course>(86, "Thomas Piskula",
                                            "702 HAM", times[0]);
    econ3412->setEnrolledStudentCount(81);
    auto econ4415 = std::make_shared<Course>(110, "Evan D Sadler",
                                            locations[1], times[2]);
    econ4415->setEnrolledStudentCount(63);
    auto econ4710 = std::make_shared<Course>(86, "Matthieu Gomez",
                                            "517 HAM", "8:40-9:55");
    econ4710->setEnrolledStudentCount(37);
    auto econ4840 = std::make_shared<Course>(108, "Mark Dean",
                                            "142 URIS", times[3]);
    ...
    auto ieor2500 = std::make_shared<Course>(50, "Uday Menon",
                                            "627 MUDD", times[0]);
    ieor2500->setEnrolledStudentCount(52);
    auto ieor3404 = std::make_shared<Course>(73, "Christopher J Dolan",
                                            "303 MUDD", times[2]);
    ieor3404->setEnrolledStudentCount(80);
    auto ieor3658 = std::make_shared<Course>(96, "Daniel Lacker",
                                            "310 FAY", times[2]);
    ieor3658->setEnrolledStudentCount(87);
    auto ieor4102 = std::make_shared<Course>(110, "Antonius B Dieker",
                                            "209 HAM", times[2]);
    ieor4102->setEnrolledStudentCount(92);
    auto ieor4106 = std::make_shared<Course>(150, "Kaizheng Wang",
                                            "501 NWC", times[2]);
    ieor4106->setEnrolledStudentCount(161);
    auto ieor4405 = std::make_shared<Course>(80, "Yuri Faenza",
                                            "517 HAV", times[0]);
    ieor4405->setEnrolledStudentCount(19);
    auto ieor4511 = std::make_shared<Course>(150, "Michael Robbins",
                                            "633 MUDD", "9:00-11:30");
    ...
    auto chem1403 = std::make_shared<Course>(120, "Ruben M Savizky",
                                            locations[1], "6:10-7:25");
    chem1403->setEnrolledStudentCount(100);
    auto chem1500 = std::make_shared<Course>(46, "Joseph C Ulichny",
                                            "302 HAV", "6:10-9:50");
    chem1500->setEnrolledStudentCount(50);
    auto chem2045 = std::make_shared<Course>(50, "Luis M Campos",
                                            "209 HAV", "1:10-2:25");
    ...
    auto chem2494 = std::make_shared<Course>(24, "Talha Siddiqui",
                                            "202 HAV", "1:10-5:00");
    chem2494->setEnrolledStudentCount(18);
    auto chem3080 = std::make_shared<Course>(60, "Milan Delor",
                                            "209 HAV", times[2]);
    chem3080->setEnrolledStudentCount(18);
    auto chem4071 = std::make_shared<Course>(42, "Jonathan S Owen",
                                            "320 HAV", "8:40-9:55");
    chem4071->setEnrolledStudentCount(29);
    auto chem4102 = std::make_shared<Course>(28, "Dalibor Sames",
                                            "320 HAV", times[2]);
    ...
    auto phys1001 = std::make_shared<Course>(150, "Szabolcs Marka",
                                            "301 PUP", times[3]);
    phys1001->setEnrolledStudentCount(125);
    auto phys1221 = std::make_shared<Course>(150, "James G. Mccann",
                                            "301 PUP", "4:10-5:25");
    phys1221->setEnrolledStudentCount(118);
    auto phys1520 = std::make_shared<Course>(400, "Victor G. Moffat",
                                            "630 MUDD", times[1]);
...
TEST_F(MyFileDatabaseUnitTests, WriteToFile) {
  testDatabase->saveContentsToFile();
  //boost::filesystem::path path("unittestsfile.bin");
  //auto size = boost::filesystem::file_size("testfile.bin");
  ASSERT_NE(-1, 0);

  MyFileDatabase newTest(0, "unittestsfile.bin");
  auto mapping = newTest.getDepartmentMapping();
  ASSERT_NE(mapping.find("PHYS"), mapping.end());
}

TEST_F(MyFileDatabaseUnitTests, ToString) {
  std::string expectedString;
  auto mapping = testDatabase->getDepartmentMapping();
  for (const auto& it : mapping) {
    expectedString += "For the " + it.first + " department:\n" +
                      it.second.display() + "\n";
  }
  ASSERT_EQ(expectedString, testDatabase->display());
}